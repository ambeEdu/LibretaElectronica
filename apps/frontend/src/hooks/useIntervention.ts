/**
 * Hook para gestionar creación de intervenciones
 */
...
export function useIntervention() {
  ...
  const [lastResponse, setLastResponse] = useState<InterventionResponse | null>(null);

  const createIntervention = useCallback(async (intervention: InterventionRequest) => {
    try {
      setLoading(true);
      setError(null);
